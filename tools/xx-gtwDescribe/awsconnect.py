...
class awsConnect:
    ...
    def __get_resource__(self, service=""):
        if not self._resources.has_key(service):
            self._resources[service] = self._session.resource(service, self._region)
        return self._resources[service]

#  client = session.client('lambda', mRegion)
    def __get_client__(self, service=""):
        if not self._clients.has_key(service):
            self._clients[service] = self._session.client(service, self._region)
