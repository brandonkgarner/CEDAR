...
from microUtils import loadConfig, yamlRead, account_replace, ansibleSetup, ansible_describe_setup, roleCleaner, writeYaml, loadServicesMap
...
class LambdaTester():
    ...
    def zip_files_move(self, testObjs, targetRole, roleFolder, zipName):
        ...
        for ff in folder_files:
            ...
            folder = ff.split("/")[0]
            extractTo = "%s/%s" % (fileFolder, bname)
            if "/" in ff:
                extractTo = "%s/%s/%s" % (fileFolder, folder, bname)
            try:
    ...
    def make_ansible(self, sendto, zipName, testObjs, targetRole, accountOrigin, accounts, role_alias=None):
        ...
        defaultVar[main_role].update(testObjs)
