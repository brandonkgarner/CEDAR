...
def ansibleInvoke(account, config, role):
    ...
    try:
        ...
        msg = "[E] error occured target:%s  file:%s error:%s" % (
            target, roleFile, e)
        print(msg)
...
def testStart(accounts, targets, allRoles, HardStop=False):
    outputs = {}
    for target in targets:
        for account, v in accounts.items():
            if target in v['all']:
                if account not in outputs:
                    outputs.update({account: {}})
                for role in allRoles:
                    account, target, result = ansibleInvoke(account, v, role)
                    outputs[account].update(
                        {role: {"name": target, "value": result}})
                if HardStop:
                    if '[E]' in result:
                        return outputs
                break
    return outputs  # {account:{role:{name:fda,value:fdas}}}
...
if __name__ == "__main__":
    ...
    results = testStart(global_accts, target_environments, role)
