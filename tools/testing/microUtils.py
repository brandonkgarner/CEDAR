...
def ansible_describe_setup(temp, target, targetRole):
    ...
    if os.path.exists(rootFolder):  # found folder target
        defaultsMain = yamlRead("%s/%s/main.yaml" % (rootFolder, "defaults"))
        taskMain = yamlRead("%s/%s/main.yaml" % (rootFolder, "tasks"))
    taskMain.append({"name": "INITIAL ROLE:%s SETUP  project VAR" % (
        targetRole), "set_fact": {"project": "{{ %s.%s }}" % (targetLabel, roleLabel)}})

    return taskMain, defaultsMain, rootFolder, targetLabel
...
def writeTXT(data, filepath, ext=None):
    if "." in filepath and not ext:
        fullpath = filepath
    else:
        fullpath = "%s.%s" % (filepath, ext)
    f = open(fullpath, 'w', encoding='utf8')
    f.write(data.strip())
    f.close()
    return fullpath
...
def yamlRead(file=None, stream=None):
    if file:
