...
class CloudFrontMolder():
    ...
    def behavior_describe(self, target, aconnect):
        ...
        if not 'DistributionList' in listCF:
            raise ValueError(
                "[E] DistributionList Not found (%s) PLEASE check / fix and try again" % (target))
    ...
    def cfront_create(self, target, aconnect, accountOrigin,  accountTarget):
        ...
        if originID is None:
            ...
            response = client.update_distribution(
                Id=originID, DistributionConfig={})
...
# python microFront.py cr-portal-dev ENVR.yaml '/Users/astro_sk/Documents/TFS/Ansible_Deployer/ansible/roles/CR-Adhoc' '/Users/astro_sk/Documents/TFS/Ansible_Deployer/ansible/roles/CR-Adhoc'
    if "help" in found and length < 3:
        ...
        if triggers is None:
            raise ValueError(
                "[E] [CF] config file [ %s ] did not load correctly.. PLEASE check / fix and try again" % (fullpath))
        ...
        print(" ## USING ## %s--> %s, role %s, account originDefinition %s, config %s, copyAnsible to %s" %
              (type_in, svc_in, accountRole, accID,  config, targetConfig))
